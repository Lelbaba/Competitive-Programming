...
template <typename DT> class polygon : public vector <point <DT>> {
public:
    using vector <point <DT>> :: vector;  
    using vector <point <DT>> :: begin;
    using vector <point <DT>> :: size;  
    point <DT> & operator [] (int idx) {
        idx %= (int) size();
...
namespace polygon_algo{
    ...
    template <typename DT> double MinimumBoundingBox(polygon <DT> P){
        ...
        for(int i = 0, l = 1, r = 1, u = 1 ;   i < n ;   i++){
            point <DT> edge = (p[i + 1]- p[i]) / sqrt(DisSq(p[i], p[i + 1]));

            for( ; (edge ^ p[r] - p[i]) < (edge ^ p[r + 1] - p[i]); r++); 
            for( ; u < r || (edge & p[u] - p[i]) < (edge & p[u + 1] - p[i]); u++) ;
            for( ; l < u || (edge ^ p[l] - p[i]) > (edge ^ p[l + 1] - p[i]); l++);
            
            double w = (edge ^ p[r] - p[i]) - (edge ^ p[l] - p[i]);
            double h = UTriArea(p[u], p[i], p[i+1]) / sqrt(DisSq(p[i], p[i+1]));
            area = min(area,w*h);
        }
        if(area > 1e19) 
            area = 0;
    ...
    template <typename DT> DT FarthestPairOfPoints(polygon <DT> p){
        ...
        {   
            for( ; UTriArea(p[i], p[i + 1], p[j + 1]) > UTriArea(p[i], p[i + 1], p[j]) ; j++ ) ;
            ans = max(ans, DisSq(p[i], p[j]));
            ans = max(ans, DisSq(p[i + 1], p[j]));
        }
